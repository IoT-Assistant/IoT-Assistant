...
sensorRulesModule.controller ("InstallTriggerActuatorSensorRuleController", function(RuleAPIService, ActuatorAPIService){
	...
	let buildTriggerActuatorSensorRule = function(sensorRuleSettings) {
		...
		let enabled = true;
		return new TriggerActuatorSensorRule(sensorMeasureThresholdSettings, null, enabled, timeBetweenTriggers, notificationType, self.actuatorSettings);
	}
	

	self.install = function(sensorRuleSettings) {
		let triggerActuatorSensorRule =  buildTriggerActuatorSensorRule(sensorRuleSettings);
		let promise = RuleAPIService.installTriggerActuatorSensorRule(triggerActuatorSensorRule);	
		return promise;
	}
